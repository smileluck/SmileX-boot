package top.zsmile.modules.generator.constant;

import java.io.Serializable;

/**
 * @author: B.Smile
 * @Date: 2022/1/22 11:31
 * @Description: Default Constants
 */
public class DefaultConstants implements Serializable {
    public static final String NAME = "SmileX-PD";
    public static final String FILE_NAME = "Database Designer Doc";
    public static final String ENCODEING = "UTF-8";
    public static final String DESCRIPTION = "Database Designer";
    public static final String NO = "N";
    public static final String YES = "Y";
    public static final String DATETIME_FORMAT = "yyyy-MM-dd HH:mm:ss";
    public static final String DATE_FORMAT = "yyyy-MM-dd";
    public static final String TIME_FORMAT = "HH:mm:ss";
    public static final String LOCALE = "zh_CN";
    public static final String VERSION = "v1.0.0";
}
